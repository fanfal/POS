...
function printInventory(input,allItems)
{
  var store = [
    {
      buy:0,
   ...
   for(i=0;i<store.length;i++)
          ...
          && store[i].send == 0)
              {
                     var total = store[i].buy*allItems[j].price;
                        string += '名字: '+allItems[j].name+
                        ',数量: '+store[i].buy.toString()+allItems[j].unit+
                        ',价格: '+allItems[j].price.toString()+'元'+
                        ',小计： '+total.toString()+'(元)\n';
                      totalValue += total;
          ...
          && store[i].send == 1)
             {
                    var total = (store[i].buy-store[i].send)*allItems[j].price;
                        string = string + '名字: '+allItems[j].name+
                        ',数量: '+store[i].buy.toString()+allItems[j].unit+
                        ',价格: '+allItems[j].price.toString()+'元'+
                        ',小计： '+total.toString()+'(元)\n';
                     sendString += '名字 :'+allItems[j].name+
                            ',数量: '+store[i].send.toString()+allItems[j].unit+'\n';
                      totalValue += total;
    ...
    var temptString = string+sendString+'-------------------------\n'+'总计： '+totalValue.toString()+'(元)\n'+'节省：'+saveValue.toString()+'（元）\n'+'**************************';
    return temptString;
}
...
var tempt = printInventory(input,allItems);
console.log(tempt);
